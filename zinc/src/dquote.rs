...
impl Lexer {
    ...
    pub(crate) fn dquote_parse(&mut self, buf: &mut Vec<u8>, endchar: u8, sub: bool) -> i32 {
        ...
        'outer: loop {
            ...
            loop {
                match c {
                    b'\\' => {
                        let Some(n) = self.input.get() else { break 'outer };
                        c = n;
                        if c != b'\n' {
                            let escapes = c == b'$'
                                || c == b'\\'
    ...
    pub(crate) fn cmd_or_math_sub(&mut self, buf: &mut Vec<u8>) -> CmdOrMath {
        ...
        if c == Some(b'\\') {
            ...
            if n != Some(b'\n') {
                ...
                self.input.unget(b'\\');
                return if self.skipcomm(buf) { CmdOrMath::Err } else { CmdOrMath::Cmd };
            }
            c = self.input.get();
        }
        if c == Some(b'(') {
        ...
        if self.skipcomm(buf) { CmdOrMath::Err } else { CmdOrMath::Cmd }
    }
    ...
    pub(crate) fn skipcomm(&mut self, buf: &mut Vec<u8>) -> bool {
        ...
        let ended = loop {
            let Some(c) = self.input.get() else { break true };
