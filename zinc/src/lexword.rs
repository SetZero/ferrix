    ...
    pub(crate) fn gettok(&mut self) -> Tok {
        ...
        loop {
            ...
            let Some(ch) = got else {
                return if self.error.is_some() { Tok::Lexerr } else { Tok::Endinput };
            };
            c = ch;
            if self.dbparens {
...
impl Lexer {
    ...
    pub(crate) fn gettokstr(&mut self, first: u8, sub: bool) -> Tok {
        ...
        let mut st = WordScan { intpos: 1, ..WordScan::default() };
        ...
        while let Some(c) = cur {
            ...
            let step = if inbl && st.in_brace_param == 0 && st.pct == 0 {
                if st.in_brace_param == 0 && !sub { Step::Break } else { Step::Add(c) }
            } else {
    ...
    #[expect(clippy::too_many_lines, reason = "one arm per character class, as in zsh's switch")]
    fn word_char(
        &mut self,
        c: u8,
        sub: bool,
        ...
        match c {
            ...
            b'|' => {
                ...
                if !self.opts.shglob || (!sub && !ibp) { Step::Add(tok::BAR) } else { Step::Add(c) }
            }
            ...
            b'>' => {
                ...
                buf.push(OUTANGPROC);
                if self.skipcomm(buf) { Step::Error } else { Step::Add(OUTPAR) }
            }
    ...
    fn word_inang(&mut self, sub: bool, st: &mut WordScan, buf: &mut Vec<u8>) -> Step {
        ...
        if !(ibp || sub) && e == Some(b'(') {
            buf.push(INANG);
            return if self.skipcomm(buf) { Step::Error } else { Step::Add(OUTPAR) };
        }
        self.input.unget_opt(e);
        if self.isnumglob() {
            buf.push(INANG);
        ...
        if ibp || sub { Step::Add(b'<') } else { Step::Break }
    }
        ...
        loop {
            ...
            while let Some(mut ch) = self.input.get() {
                ...
                if strquote && ch == b'\\' {
                    ...
                    ch = n;
                    buf.push(if ch == b'\\' || ch == b'\'' { BNULL } else { b'\\' });
        ...
        loop {
            ...
            match ch {
                ...
                b'\\' => match self.input.get() {
                    ...
                    Some(n) => {
                        buf.push(if matches!(n, b'`' | b'\\' | b'$') { BNULL } else { b'\\' });
                        buf.push(n);
                    }
                    None => {
                        st.unmatched = b'`';
...
fn assignment_name_end(buf: &[u8]) -> usize {
    ...
    if buf.first().is_some_and(u8::is_ascii_digit) {
        ...
        while buf.get(t).is_some_and(|&b| b.is_ascii_alphanumeric() || b == b'_') {
            t += 1;
        }
