...
pub(crate) enum CmdKind {
    ...
    /// are redirections).
    Simple { assigns: Vec<Assign>, words: Vec<Word> },
    /// `typeset` and its relatives, whose arguments may be array assignments.
    Typeset { assigns: Vec<Assign>, words: Vec<Word>, args: Vec<Assign> },
    /// `( list )`.
    Subsh(List),
    /// `{ list }`.
    Cursh(List),
    ...
    For { vars: Vec<Word>, words: Option<Vec<Word>>, body: List },
    /// `for (( init; cond; step ))`.
    ForArith { init: Word, cond: Word, step: Word, body: List },
    /// `select name in words`.
    Select { var: Word, words: Option<Vec<Word>>, body: List },
    ...
    /// once with `args`.
    FuncDef { names: Vec<Word>, body: Rc<List>, tracing: bool, args: Vec<Word> },
...
pub(crate) enum Cond {
    ...
    Module { name: Word, args: Vec<Word>, infix: bool },
}
