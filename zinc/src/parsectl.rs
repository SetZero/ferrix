...
impl Parser<'_> {
    ...
    pub(crate) fn par_for(&mut self) -> PResult<CmdKind> {
        ...
        if self.tok() == Tok::Dinpar {
            ...
            return Ok(CmdKind::ForArith { init, cond, step, body });
        }
        self.lx.infor = 0;
    ...
    pub(crate) fn par_if(&mut self) -> PResult<CmdKind> {
        ...
        Ok(CmdKind::If { branches, otherwise })
    }
    ...
    pub(crate) fn par_subsh(&mut self, zsh_construct: bool) -> PResult<CmdKind> {
        ...
        self.next();
        let body = self.par_list()?;
        let close = if otok == Tok::Inpar { Tok::Outpar } else { Tok::Outbrace };
        if self.tok() != close {
            return self.error();
        }
        ...
        Ok(if otok == Tok::Inpar { CmdKind::Subsh(body) } else { CmdKind::Cursh(body) })
    }
    ...
    pub(crate) fn par_funcdef(&mut self, redirs: &mut Vec<Redir>) -> PResult<CmdKind> {
        ...
        Ok(CmdKind::FuncDef { names, body: Rc::new(body), tracing, args })
    }
    ...
    fn par_cond_2(&mut self) -> PResult<Cond> {
        ...
        let dble = s1.as_deref().is_some_and(|s| is_dash(s.first()) && s.len() == 2);
        if self.tok() != Tok::String {
            if let Some(s1) = s1
                && self.tok() != Tok::Lexerr
                && !dble
        ...
        if matches!(self.tok(), Tok::Inang | Tok::Outang) {
            let op = if self.tok() == Tok::Inang { CondOp::StrLt } else { CondOp::StrGt };
            loop {
                self.next();
                if !self.cond_sep() {
                    break;
    ...
    fn cond_double(&mut self, a: Word, b: Word) -> PResult<Cond> {
        ...
        Ok(Cond::Module { name: a, args: vec![b], infix: false })
    }
    ...
    fn cond_triple(&mut self, a: Word, b: Word, c: Word) -> PResult<Cond> {
        ...
        if is_dash(b.first()) {
            ...
            return Ok(match num {
                ...
                None => Cond::Module { name: b, args: vec![a, c], infix: true },
            });
        }
        if is_dash(a.first()) && a.len() > 1 {
            return Ok(Cond::Module { name: a, args: vec![b, c], infix: false });
        }
    ...
    fn cond_multi(&mut self, a: Word, args: Vec<Word>) -> PResult<Cond> {
        ...
        Ok(Cond::Module { name: a, args, infix: false })
    }
}
