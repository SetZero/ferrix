...
impl<'a> Parser<'a> {
    ...
    /// Read the next token. zsh's parser calls `zshlex`, not `ctxtlex`: it
    /// sets `incmdpos` itself, and the automatic version reads a closing
    /// `esac` after a newline as a reserved word where a string is expected.
    pub(crate) fn next(&mut self) {
        self.lx.zshlex(self.env);
    }
    ...
    pub(crate) fn parse_event(&mut self) -> PResult<Option<List>> {
        ...
        loop {
            ...
            let Some(sublist) = self.par_sublist()? else {
                return self.error();
            };
    ...
    fn par_sublist(&mut self) -> PResult<Option<Sublist>> {
        let Some(first) = self.par_sublist2()? else {
            return Ok(None);
        };
        let mut rest = Vec::new();
        while matches!(self.tok(), Tok::Dbar | Tok::Damper) {
            let op = if self.tok() == Tok::Dbar {
                AndOr::Or
            } else {
                AndOr::And
            };
        ...
        loop {
            match self.tok() {
                ...
                Tok::Envarray => {
                    ...
                    args.push(Assign {
                        name,
                        value: AssignValue::Array(list),
                        append: false,
                    });
                    self.next();
                }
                Tok::Inoutpar => {
                    return self.par_simple_funcdef(words, &assigns, postassigns, redirs);
                }
                _ => break,
            }
        }
    ...
    fn par_simple_funcdef(
        ...
        let body = if self.tok() == Tok::Inbrace {
            ...
            let Some(cmd) = self.par_cmd(anonymous)? else {
                return self.error();
            };
            if anonymous {
                self.lx.incmdpos = false;
