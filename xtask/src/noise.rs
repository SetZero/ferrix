...
impl Filter {
    ...
    /// Wait for the stream to end, and say how many lines were hidden.
    ///
    /// A thread that panicked hid nothing worth saying, and its panic has
    /// already been printed, so the count is zero rather than a failure of
    /// the boot it was watching.
...
pub(crate) fn run(mut command: Command, description: &str) -> Result<()> {
    ...
    report(filter.map_or(0, Filter::finish));
