#!/usr/bin/env bash
# Print the DRM/KMS numbers and layouts from the UAPI headers, at both widths.
#
# Needs a Linux host with the kernel's UAPI headers (linux-libc-dev), gcc,
# arm-linux-gnueabihf-gcc and qemu-arm. Writes drm-64.txt and drm-32.txt
...
gcc -O0 -Wall -Werror -o "$work/drm-64" "$here/drm.c"
"$work/drm-64" > "$here/drm-64.txt"
...
arm-linux-gnueabihf-gcc -O0 -Wall -Werror -static -I"$work/include" \
    -o "$work/drm-32" "$here/drm.c"
qemu-arm "$work/drm-32" > "$here/drm-32.txt"
