...
fn check_recovery(run: &Run, crash: usize, rng: &mut Rng) {
    ...
    let mut volume = match WriteVolume::open(device.clone()) {
        Ok(volume) => volume,
        Err(error) => panic!("a crash at {crash} left a volume that will not open: {error}"),
    };
    volume.commit().unwrap_or_else(|e| panic!("a crash at {crash} left a volume that will not commit: {e}"));
    check(&volume.device);
    for &ino in &run.inodes {
        ...
        let last_completed = run
            .promises
            .iter()
            .filter(|promise| promise.ino == ino && promise.at <= crash)
            .next_back();
