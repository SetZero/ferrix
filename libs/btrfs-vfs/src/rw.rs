...
impl<D: WriteHandle> Node<D> {
    ...
    fn write_back(&self, volume: &mut WriteVolume<D>) -> Result<()> {
        ...
        let (dirty, held) = {
            ...
            if let Some((written, still)) = store.as_ref().and_then(|pages| pages.mapped_writes()) {
                dirty.extend(written);
                still_mapped = still;
            }
            // Written back from here on, so the mount lets go, unless a
            // mapping may still write; a write after this marks its pages
            // under the same lock and holds it again.
            let held = if still_mapped {
                None
            } else {
...
impl<D: WriteHandle> Inode for Node<D> {
    ...
    /// the writeback that finds none left that may lets go.
