...
fn a_write_answers_the_offset_just_past_it() {
    ...
    assert_eq!(file.write_at(4096, &[2u8; 100], false).unwrap(), (100, 4196));
...
fn what_is_written_outlives_the_last_reference_to_its_file() {
    ...
    // file as long as it was then. Found by `cargo xtask test-selfhost`,
    // whose rustc read its own object files back short.
    let disk = Disk::new(BLANK);
    let fs = mount(&disk);
