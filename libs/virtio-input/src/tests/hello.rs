...
/// What `read_hello` makes of a device answering `answers`, with the ways the
/// driver broke the protocol on the way. A test that expects a description
/// calls [`described`], which requires there to have been none.
fn hello_of(answers: Vec<Answer>) -> (Result<(Hello, bool), InputError>, Vec<&'static str>) {
    let bus = Bus::new();
    ...
    let errors = device.borrow().protocol_errors.clone();
    (result, errors)
}

/// The description of a device that has one, the driver having kept to the
/// protocol asking for it.
fn described(answers: Vec<Answer>) -> (Hello, bool) {
    let (result, errors) = hello_of(answers);
    assert!(errors.is_empty(), "{errors:?}");
    result.expect("described")
}
...
fn the_keyboard_is_described_as_qemu_declares_it() {
    let (hello, clipped) = described(keyboard());
    assert!(!clipped);
    ...
    assert!(
        hello
            .axes
            .iter()
            .all(|range| *range == AxisRange::default())
    );
    ...
    assert!(
        caps.bits.has_type(EV_SYN),
        "the core publishes EV_SYN itself"
    );
}

#[test]
fn the_mouse_and_tablet_are_described_with_their_axes() {
    let (mouse, _) = described(mouse());
...
fn multitouch_is_declared_and_the_core_leaves_it_out() {
    let (hello, _) = described(multitouch());
...
fn bits_past_a_kinds_max_are_left_out_and_said_to_be() {
    ...
    let (hello, clipped) = described(vec![(0x11, 0x01, vec![0xff; 128])]);
    assert!(clipped);
    ...
    let (hello, clipped) = described(vec![(0x11, 0x15, vec![0xff; 16]), (0x11, 0x12, vec![0x0f])]);
    assert!(!clipped);
...
fn a_device_without_names_or_ids_is_described_without_them() {
    let (hello, _) = described(vec![(0x11, 0x01, vec![0, 0, 0, 0b0100_0000])]);
