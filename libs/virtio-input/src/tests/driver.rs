...
fn parts(
    bus: &Rc<Bus>,
    device: &Rc<RefCell<Device>>,
) -> (Parts<Handle, Region, Region>, Rc<RefCell<usize>>) {
...
fn test_inputs_sequence_reaches_the_core_as_whole_reports() {
    ...
    let mut sessions = [&keyboard, &tablet]
        .map(|rig| Session::accept(&rig.driver.hello(0), &Hello::HANDLE_RIGHTS).expect("accepted"));
    for value in [1, 0] {
        keyboard
            .device
            .borrow_mut()
            .send(event(EV_KEY, KEY_A, value));
...
fn the_queue_is_refilled_before_anything_is_forwarded() {
    ...
    for index in 0..33 {
        rig.device
            .borrow_mut()
            .send(event(EV_KEY, KEY_A, (index % 2) ^ 1));
...
fn what_the_core_would_not_publish_is_dropped_here() {
    ...
    assert_eq!(
        rig.driver
            .pop_events()
            .map(|events| events.as_slice().to_vec()),
...
fn a_full_batch_leaves_completions_in_the_ring_until_messages_are_taken() {
    ...
    for round in 0..5 {
        for _ in 0..64 {
            rig.device
                .borrow_mut()
                .send(event(EV_KEY, KEY_A, (sent % 2) ^ 1));
            sent += 1;
        }
        let drained = rig.driver.on_interrupt().expect("sound");
        taken += drained.taken;
    ...
    // The driver takes a completion while two events fit, and each one
    // pushes a single event, so the last take leaves the batch one short of
    // its capacity: the room a SYN_REPORT would have needed.
    assert_eq!(taken, CAPACITY - 1, "one short of the batch");
    let drained = rig.driver.on_interrupt().expect("sound");
    assert!(drained.more);
    assert_eq!(drained.taken, 0);

    // Taking messages makes room, and nothing the device wrote was lost.
    let mut forwarded = 0;
    loop {
        while let Some(events) = rig.driver.pop_events() {
    ...
    assert_eq!(
        rig.driver.pending(),
        0,
        "the ring and the batch are drained"
    );
    // The device never sent a SYN_REPORT, so this is one report as long as
    // the run: the batch cut it once at `REPORT_EVENTS` and put a
    // `SYN_REPORT` of its own in, which is the only event out that the
    // device did not write.
    assert_eq!(rig.driver.split_reports(), 1);
    assert_eq!(
        forwarded,
        rig.device.borrow().delivered + 1,
        "every event the device wrote came out, and the cut's SYN_REPORT with them"
    );
    // The round that filled the batch left one completion in the used ring
    // and refilled the other 63 buffers, so the last of the next round's 64
    // events found none and the device dropped it. `sent` counts what was
    // offered to the device, not what it took.
    assert_eq!(sent, 320);
    assert_eq!(rig.device.borrow().dropped_reports, 1);
    assert_eq!(rig.device.borrow().delivered, 319);
}
...
fn bring_up_failures_hand_the_memory_back() {
    ...
    let (failure, _) = fail(
        |device| device.offered = FEATURE_ACCESS_PLATFORM,
        keyboard(),
        1,
    );
    assert_eq!(
...
fn a_smaller_queue_is_a_power_of_two_and_a_device_that_will_not_reset_keeps_the_memory() {
    ...
    // The process leaks these on purpose: a device that did not reset may
    // still write into them. Here they are the test's own pages, with no
    // `Drop`, so letting them fall out of scope leaks them the same way.
