...
impl Device {
    ...
    pub(super) fn new(bus: Rc<Bus>, answers: Vec<Answer>) -> Self {
        let longest = answers.iter().map(|answer| answer.2.len()).max().unwrap_or(0);
        Self {
            bus,
            ...
            DEVICE_STATUS => {
                if value as u8 & STATUS_DRIVER_OK != 0 {
                    if self.ring.is_none() {
                        self.protocol_errors.push("DRIVER_OK before the event queue");
                    }
                    self.active = true;
                }
    ...
    fn enable(&mut self) {
        if self.queue_select != 0 {
            self.protocol_errors.push("a queue other than the event queue enabled");
            return;
        }
        if self.status & STATUS_FEATURES_OK == 0 {
            self.protocol_errors.push("a queue enabled before FEATURES_OK");
        }
    ...
    fn write(&mut self, event: Event) {
        ...
        let [descriptor] = descriptors[..count] else {
            self.protocol_errors.push("an event chain of more than one buffer");
            return;
        };
        if !descriptor.is_device_writable() {
            self.protocol_errors.push("an event buffer the device cannot write");
        }
