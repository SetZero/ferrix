    ...
    loop {
            ...
            && pace.due(Instant::now(), refresh_ns(&screens))
        {
            ...
            if reported.elapsed() >= FRAME_REPORT {
                report(&format!(
                    "hyprix: frames {drawn} slowest of the last {counted} {} us, all of them {} us ({}; drew {} px in {} rects, showed {} px)",
                    since.as_micros(),
                    spent.as_micros(),
        ...
        let timeout = [frame_wait, idle_wait, deadline_wait, rescan_wait]
            .into_iter()
            .flatten()
            .min();
