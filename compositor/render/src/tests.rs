...
fn the_tiled_frame_has_its_gaps_and_borders_on_exact_pixels() {
    ...
    assert_eq!(at(505, 300), shown(Pattern::Checkerboard.pixel(484, 279, 726)));
    ...
    assert_eq!(at(100, 746), shown(Pattern::Checkerboard.pixel(79, 725, 726)));
...
fn a_translucent_border_is_blended_once_at_its_corners() {
    ...
    canvas.border(Rect::new(2, 2, 4, 4), 1, Color(0x80FF_0000), &Damage::full(8, 8));
...
fn a_padded_surface_draws_as_a_tight_one() {
    for format in [Format::Xrgb8888, Format::Argb8888] {
        ...
        a.composite(&Surface::new(&tight, width, height, width * 4, format).unwrap(), rect, &full);
        b.composite(&Surface::new(&padded, width, height, stride, format).unwrap(), rect, &full);
...
fn presenting_honours_a_stride_wider_than_the_pixels() {
    ...
    canvas.fill(Rect::new(1, 1, 3, 2), Color(0xFFAB_CDEF), &Damage::full(width, height));
    ...
    canvas.present(&mut target, &Damage::full(width, height)).unwrap();
    for y in 0..height {
        ...
        if y + 1 < height {
            assert_eq!(row[width as usize * 4..stride as usize], [0xEE; 8], "padding");
        }
    }
}
...
fn presenting_writes_only_the_damage_and_needs_the_canvas_size() {
    ...
    canvas.present(&mut target, &Damage::from(Rect::new(1, 2, 2, 1))).unwrap();
...
fn drawing_is_clipped_to_the_damage_and_reports_what_it_wrote() {
    ...
    canvas.fill(Rect::new(20, 20, 5, 5), Color(0xFFFF_FFFF), &Damage::full(10, 10));
...
fn damage_is_a_region_of_disjoint_rectangles() {
    ...
    // Past the limit it becomes its bounding box.
    let mut many = Damage::new();
    for i in 0..40 {
        many.add(Rect::new(i * 2, 0, 1, 1));
    }
    assert_eq!(many.rects(), [Rect::new(0, 0, 79, 1)]);
}
...
fn a_frame_with_partial_damage_leaves_the_rest_alone() {
    ...
    let bind = compositor_config::parse("t.conf", "bind = SUPER, H, movefocus, l\n", &mut NoSources)
        .config
        .binds
        .remove(0);
    let dispatcher = compositor_layout::Dispatcher::from_bind(&bind).unwrap();
    assert!(!state.dispatch(&dispatcher).is_empty());
    ...
    for (index, (old, new)) in first.chunks(4).zip(canvas.data().chunks(4)).enumerate() {
        let (x, y) = (index as i64 % i64::from(WIDTH), index as i64 / i64::from(WIDTH));
        if old != new {
            assert!(damage.contains(x, y), "({x}, {y}) changed outside the damage");
        }
    }
...
fn a_layout_on_a_monitor_away_from_the_origin_is_drawn_in_its_coordinates() {
    ...
    assert_eq!(canvas.pixel(20, 20), Some(0xFFFF_FFFF), "the focused border");
    assert_eq!(canvas.pixel(21, 21), Some(0xFF11_1111), "no surface yet");
    assert_eq!(canvas.pixel(19, 20), Some(0xFF11_1111));
    assert_eq!(
        damage_between(&layout, &MonitorLayout { windows: Vec::new(), ..layout.clone() }, (1920, 100)),
...
fn buffers_are_checked_when_described() {
    ...
    assert_eq!(Format::from_wl_shm(Format::Xrgb8888.wl_shm()), Some(Format::Xrgb8888));
...
fn the_patterns_are_what_their_docs_say() {
    ...
    // Bottom half, alpha 0xC0: 16 × 192 / 255 = 12.05, 128 × 192 / 255 = 96.4.
    assert_eq!(Pattern::Gradient.pixel(16, 16, 20), 0xC00C_0C60);
    // Premultiplied: no channel above alpha.
    for y in 0..40 {
        let pixel = Pattern::Gradient.pixel(1000, y * 16, 640);
        let alpha = pixel >> 24;
        assert!([16, 8, 0].iter().all(|shift| (pixel >> shift) & 0xFF <= alpha));
    }
}
