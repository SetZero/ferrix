...
pub(crate) use aarch64::{
    ...
    USER_TEST_PROGRAM, USER_TEST_STATUS, advance_past_breakpoint, breakpoint, classify, console,
    counter_hz, counter_now, cpu_local, cpu_local_register, decode_syscall, describe_cpus,
    disable_interrupts, drop_identity_map, enable_interrupts, flush_tlb, frame_pointer, halt,
    hardware_id, identity_root, init_console, init_interrupts, init_traps, install_user_root,
    ipi_irq, prepare_stack, prepare_user_root, read_console_byte, report_trap, run_user,
    send_ipi_to_others, service_interrupts, set_cpu_local, shutdown, switch_to, system_call,
    timer_arm, timer_disarm, timer_irq, uninstall_user_root, wait_for_interrupt, wait_for_work,
};
#[cfg(target_arch = "arm")]
