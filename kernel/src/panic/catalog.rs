...
/// For `wait_for_everyone` in `smp.rs`, when a TLB shootdown times out.
pub(crate) static SHOOTDOWN_TIMEOUT: Explanation = Explanation {
    ...
    meaning: "When a kernel mapping is removed or made less permissive on x86-64, \
              ...
              processor that has not answered may still translate through the old entry, \
              so the memory behind it cannot safely be freed or the narrowed permission \
              relied on, and the kernel stops instead. AArch64 and ARMv7-A invalidate \
              every processor's TLB in hardware and never wait here.",
    causes: &[
...
/// For `wait_for_everyone` in `smp.rs`, when a grace period times out.
pub(crate) static GRACE_PERIOD_TIMEOUT: Explanation = Explanation {
    ...
    meaning: "`synchronize` waits for a grace period by interrupting every other online \
              processor and waiting up to five seconds for each to take the interrupt, \
              which none can do inside a read-side section, because a section masks \
    ...
    causes: &[
        "A read-side section on the named processor ran for more than five seconds, or \
...
pub(crate) static STAGE7_SYSCALLS: Explanation = Explanation {
    ...
    causes: &[
        ...
        "Two spinning programs on one processor were never both preempted in three attempts. \
         One attempt can be the host: a stall while one program runs is charged to it as \
         service, and the scheduler then lets the other run its whole loop. Each attempt \
         prints both preemption and switch counts, the processor's worst overrun and its \
         preemption-lock count. Being switched to twice is not evidence: a program never \
         preempted shows that too, from its start and its first write.",
    ],
