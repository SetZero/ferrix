...
use ferrix_blkring::{
    Completed, Device, Ending, KernelSide, Location, Message, Refusal, RingMemory, Slot, Status,
    Submission, SubmitError, Wait,
};
...
use crate::{arch, mm, sched, timer};

use standin::{BlockDevice, BlockRefused, BlockRegistration, register_block};
...
/// accepted, since nothing yet says its device was reset.
...
pub(crate) fn create(node: &Arc<DeviceNode>) -> Result<Arc<Endpoint>, CreateError> {
    ...
    if sched::spawn("block ring", run, id, ferrix_sched::NICE_0_WEIGHT).is_err() {
        let _ = take_start(id);
        unclaim(id);
...
fn receive_hello(control: &Endpoint) -> Option<object::channel::ChannelMessage> {
    ...
    loop {
        ...
        let ready = control.waiters().wait_until_deadline(
            || control.signals().intersects(Signals::READABLE | Signals::PEER_CLOSED),
            deadline,
        );
        if !ready {
            return None;
...
impl RingDisk {
    ...
    fn read_request(&self, sector: u64, count: u32, out: &mut [u8]) -> Result<(), Errno> {
        ...
        let _ = self.done.wait_until_deadline(|| self.finished(id), deadline);
        let mut state = self.state.lock();
...
impl Serving<'_> {
    ...
    fn sleep(&mut self) {
        ...
        let _ = port.waiters().wait_until_deadline(|| !port.is_empty(), deadline);
...
/// Stand-ins for ferrix-e6's `fs::block::BlockDevice` and
/// `devfs::register_block`, with the signatures agreed with them, until those
/// land. **Not for landing:** the ring is rebased onto the real ones.
mod standin {
    use alloc::sync::Arc;
    use alloc::vec::Vec;
    use core::fmt;
    ...
    /// A disk the kernel reads whole sectors from.
    pub(crate) trait BlockDevice: Send + Sync + fmt::Debug {
        /// Whole sectors from `sector` into `buf`. Called with no spin lock
        /// held; may sleep on the device. A device that has gone away returns
        /// EIO, never blocks forever, never panics.
        fn read(&self, sector: u64, buf: &mut [u8]) -> Result<(), Errno>;
    ...
    /// Why a node was not published.
    #[derive(Clone, Copy, PartialEq, Eq, Debug)]
    pub(crate) enum BlockRefused {
        /// The name is not one a node may have.
        InvalidName,
        /// A node of that name is published.
        NameInUse,
        /// A node of that number is published.
        NumberInUse,
    }

    /// A published node, unpublished when dropped.
    #[derive(Debug)]
    ...
    /// Published names and numbers.
    static PUBLISHED: SpinLock<Vec<(Vec<u8>, u32, u32)>> = SpinLock::new(Vec::new());

    /// Publish `device` as `/dev/<name>`.
    pub(crate) fn register_block(
        name: &[u8],
        major: u32,
        minor: u32,
        device: Arc<dyn BlockDevice>,
    ) -> Result<BlockRegistration, BlockRefused> {
        let _ = device;
        if name.is_empty() {
            return Err(BlockRefused::InvalidName);
        }
        let mut published = PUBLISHED.lock();
        if published.iter().any(|(taken, _, _)| taken == name) {
            return Err(BlockRefused::NameInUse);
        }
        if published.iter().any(|&(_, a, b)| (a, b) == (major, minor)) {
            return Err(BlockRefused::NumberInUse);
        }
        published.push((name.to_vec(), major, minor));
    ...
    impl Drop for BlockRegistration {
        fn drop(&mut self) {
            PUBLISHED.lock().retain(|(taken, _, _)| *taken != self.name);
        }
    }
}
