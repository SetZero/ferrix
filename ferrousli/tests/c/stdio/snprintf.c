/* Adapted from libc-test's src/functional/snprintf.c (MIT). */
...
int main(void)
{
	...
	for (j=0; int_tests[j].fmt; j++) {
		...
		if (i != (int)strlen(int_tests[j].expect)) {
			t_error("snprintf(b, sizeof b, \"%s\", %d) returned %d wanted %d\n",
				int_tests[j].fmt, int_tests[j].i, i, (int)strlen(int_tests[j].expect));
		}
	...
	for (j=0; fp_tests[j].fmt; j++) {
		...
		if (i != (int)strlen(fp_tests[j].expect)) {
			t_error("snprintf(b, sizeof b, \"%s\", %f) returned %d wanted %d\n",
				fp_tests[j].fmt, fp_tests[j].f, i, (int)strlen(fp_tests[j].expect));
		}
