/*
 * Adapted from libc-test's src/functional/memstream.c (MIT). Its fscanf call,
 * which waits for scanf, is replaced by reading the same five bytes with getc.
 */
#define _GNU_SOURCE
#include <stdlib.h>
#include <unistd.h>
...
int main(void)
{
	...
	/* Beyond libc-test: a write past a fixed buffer, and a buffer of fmemopen's own. */
	TEST_E(f = fmemopen(buf, 4, "w"));
	TEST(i, (int)fwrite("abcdef", 1, 6, f), 4, "%d != %d");
	TEST(i, ferror(f) != 0, 1, "%d != %d");
	fclose(f);
	TEST_E(f = fmemopen(NULL, 16, "w+"));
	TEST_E(fputs("own", f) >= 0);
	rewind(f);
	TEST(i, getc(f), 'o', "%d != %d");
	fclose(f);
	errno = 0;
	TEST(i, fmemopen(buf, 10, "z") == NULL && errno == EINVAL, 1, "%d != %d");
	return t_status;
}
