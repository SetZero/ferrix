/*
 * Adapted from libc-test's src/functional/qsort.c and the random helpers in
 * src/common/rand.c (MIT). Its t_error reports became CHECKs.
 */
...
/* From libc-test's src/common/rand.c. */
...
static void t_randseed(uint64_t s)
{
	seed = s;
}
/* uniform random in [0,n), n > 0 must hold */
static uint64_t t_randn(uint64_t n)
{
	uint64_t r, m;
...
static void t_shuffle(uint64_t *p, size_t n)
{
	size_t r;
	uint64_t t;
...
#define T(a, a_sorted) do { \
	...
	CHECK(memcmp(p, a_sorted, sizeof p) == 0); \
} while(0)
