...
yet and aborts: 29 functions in `src/stubs.rs`. That list only shrinks, and
...
| Password hashing | `crypt` and `crypt_r`: `$1$` MD5, `$5$` SHA-256 and `$6$` SHA-512, with musl's key, salt and `rounds=` limits, checked against musl's and Drepper's vectors | the traditional DES hash and `$2*$` blowfish, which give `"*"` until they are here; `encrypt`, `setkey` |
