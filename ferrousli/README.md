...
| `stdlib.h` | `exit`, `_Exit`, `atexit` without a limit, `quick_exit` and its separate 32-handler stack, `abort`; the environment functions including secure execution; `getsubopt`; `a64l` and `l64a`; `strtol` and `strtod` families, correctly rounded for `float`, `double` and x87 `long double`, with glibc's `__isoc23_` names; `qsort`, `qsort_r`, `bsearch`; `abs` and `div` families; `rand`, `random` and `rand48` families | `ecvt`, `fcvt`, `gcvt`; NaN payloads and rounding modes in parsing |
...
| Password hashing | `crypt` and `crypt_r`: traditional and BSDi extended DES, `$1$` MD5, `$5$` SHA-256 and `$6$` SHA-512, with musl's key, salt and `rounds=` limits, checked against musl's, libxcrypt's and Drepper's vectors; `encrypt` and `setkey` | `$2*$` blowfish, which gives `"*"` until it is here |
