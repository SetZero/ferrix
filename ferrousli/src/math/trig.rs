...
pub(crate) fn rem_pio2_large(x: &[f64; 3], nx: i32, e0: i32, prec: i32) -> (i32, [f64; 2]) {
    ...
    let mut j = jv - jx;
    for i in 0..=jx + jk {
        put(&mut f, i, if j < 0 { 0.0 } else { f64::from(at(&IPIO2, j)) });
        j += 1;
    }
...
mod tests {
    ...
    fn sin_rounds_as_musl_does_upward_and_downward() {
        ...
        let cases = [
            (FE_DOWNWARD, hexf64!("-0x1.3773fbd46be7dp+3"), hexf64!("0x1.368e5c4f62bfap-2")),
            (FE_UPWARD, hexf64!("-0x1.394540722b124p+1"), hexf64!("-0x1.478cb03f39e3bp-1")),
            (FE_DOWNWARD, hexf64!("-0x1.3954425bfd23dp+204"), hexf64!("-0x1.3f95241253e1p-2")),
            (FE_UPWARD, hexf64!("-0x1.5ea04e4bf8851p+9"), hexf64!("0x1.40d64b11aedd8p-1")),
        ];
        for (mode, x, want) in cases {
            let (got, raised) = mtest::under(mode, || sin(x));
            assert_eq!(got.to_bits(), want.to_bits(), "sin({x:e}) in mode {mode:#x}");
