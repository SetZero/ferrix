...
//! ```
//!
//! The table was converted from `exp_data.c` by a script, digit for digit.
//! musl's x86-64 build has no `__FP_FAST_FMA`, no `TOINT_INTRINSICS` and no
//! `EXP_USE_TOINT_NARROW`, so the code kept is the code without them.

use crate::math::support::{barrier, force_eval, hexf64, oflow, uflow};

/// `EXP_TABLE_BITS`: the table covers 2^(k/N) for N = 2^7.
pub(crate) const TABLE_BITS: u32 = 7;
/// `N`, the number of entries.
pub(crate) const N: u64 = 1 << TABLE_BITS;
/// N/ln2.
pub(crate) const INV_LN2_N: f64 = hexf64!("0x1.71547652b82fep0") * 128.0;
/// The high bits of -ln2/N.
pub(crate) const NEG_LN2_HI_N: f64 = hexf64!("-0x1.62e42fefa0000p-8");
/// The rest of -ln2/N.
pub(crate) const NEG_LN2_LO_N: f64 = hexf64!("-0x1.cf79abc9e3b3ap-47");
/// Added before k's bits are read out of kd, without `TOINT_INTRINSICS`.
pub(crate) const SHIFT: f64 = hexf64!("0x1.8p52");
/// The coefficients of exp(r) - 1 after r: `poly[0]` to `poly[3]`.
pub(crate) const C2: f64 = hexf64!("0x1.ffffffffffdbdp-2");
/// See [`C2`].
pub(crate) const C3: f64 = hexf64!("0x1.555555555543cp-3");
/// See [`C2`].
pub(crate) const C4: f64 = hexf64!("0x1.55555cf172b91p-5");
/// See [`C2`].
pub(crate) const C5: f64 = hexf64!("0x1.1111167a4d017p-7");

/// 2^(k/N) ~= H\[k\]·(1 + T\[k\]) for k in [0, N): entry 2k holds the bits of
/// T\[k\], and entry 2k + 1 the bits of H\[k\] less (k << 52)/N.
static TABLE: [u64; 2 * N as usize] = [
...
pub(crate) const fn top12(x: f64) -> u32 {
    (x.to_bits() >> 52) as u32
}
...
fn specialcase(tmp: f64, sbits: u64, ki: u64) -> f64 {
    ...
    if y < 1.0 {
        ...
        lo = 1.0 - hi + y + lo;
        y = (hi + lo) - 1.0;
        // Avoid -0.0 when rounding downward.
        if y == 0.0 {
            y = 0.0;
        }
...
mod tests {
    ...
    #[test]
    fn exp_matches_libc_test() {
        let files = ["crlibm/exp.h", "ucb/exp.h", "sanity/exp.h", "special/exp.h"];
        mtest::d_d("exp", &files, |x| exp(x), Rules::ULP, &[]);
    }

    #[test]
    fn the_table_starts_and_ends_as_musls() {
        assert_eq!(table(0), 0);
        assert_eq!(table(1), 0x3ff0_0000_0000_0000);
        assert_eq!(table(2 * N - 2), 0x3c53_05c1_4160_cc89);
        assert_eq!(table(2 * N - 1), 0x3fef_f3c2_2b8f_71f1);
        assert_eq!(table(2 * N), 0);
    }
}
