...
mod tests {
    ...
    fn second<F: mtest::Float>(verdict: Verdict, row: &Row, what: &str, got: F, want: F) -> Verdict {
    ...
    fn exponents_match_libc_test() {
        ...
        mtest::d_i("ilogb", &files, |x| i64::from(ilogb(x)), Rules::INTEGER, &[]);
        let files = ["sanity/ilogbf.h", "special/ilogbf.h"];
        mtest::d_i("ilogbf", &files, |x| i64::from(ilogbf(x)), Rules::INTEGER, &[]);
    }
    ...
    fn differences_and_extremes_match_libc_test() {
        let exact = Rules::EXACT;
        mtest::dd_d("fdim", &["sanity/fdim.h", "special/fdim.h"], |x, y| fdim(x, y), exact, &[]);
        mtest::dd_d("fdimf", &["sanity/fdimf.h", "special/fdimf.h"], |x, y| fdimf(x, y), exact, &[]);
        mtest::dd_d("fmax", &["sanity/fmax.h", "special/fmax.h"], |x, y| fmax(x, y), exact, &[]);
        mtest::dd_d("fmaxf", &["sanity/fmaxf.h", "special/fmaxf.h"], |x, y| fmaxf(x, y), exact, &[]);
        mtest::dd_d("fmin", &["sanity/fmin.h", "special/fmin.h"], |x, y| fmin(x, y), exact, &[]);
        mtest::dd_d("fminf", &["sanity/fminf.h", "special/fminf.h"], |x, y| fminf(x, y), exact, &[]);
    }
