...
fn add_max(a: u32, b: u32) -> u32 {
    a.checked_add(b).filter(|&t| t < INFINITE).unwrap_or(INFINITE)
}
...
fn mul_max(a: u32, n: u32) -> u32 {
    if a == 0 {
        ...
        a.checked_mul(n).filter(|&t| t < INFINITE).unwrap_or(INFINITE)
    }
}
...
impl Program {
    ...
    fn emit_any(&mut self, lo: u32, hi: u32) -> Mem<()> {
        ...
        let optional = hi.checked_sub(lo).filter(|&n| lo == mandatory && n <= MAX_BACKREF_COPIES);
    ...
    pub(super) fn backward(&mut self, start: u32, end: u32, lo: usize, to: usize) -> Mem<Positions> {
        ...
        loop {
            ...
            while let Some(q) = self.cur.nth(i) {
                i += 1;
                for &p in prog.predecessors(q) {
                    if p < start || p >= end || p + 1 != q {
                        continue;
                    }
                    let consuming = matches!(prog.inst(p), Inst::Byte(_) | Inst::Set(_) | Inst::Any);
...
mod tests {
    ...
    fn positions_span_many_words_both_ways() {
        ...
        for (set, lowest) in [(&up, 10), (&down, 5)] {
            ...
            assert_eq!(set.highest_at_most(74), Some(if lowest == 10 { 10 } else { 10 }));
