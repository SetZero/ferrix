...
pub unsafe extern "C" fn sem_post(sem: *mut Sem) -> c_int {
    ...
    if value < 0 {
        futex::wake(&sem.value, if waiters > 1 { 1 } else { -1 }, private);
    }
    0
}
...
unsafe fn clockwait(sem: *mut Sem, clock: c_int, at: *const Timespec) -> c_int {
    ...
    while !sem.try_take() {
        ...
        // outlives the wait.
        unsafe { cancel::_pthread_cleanup_push(&raw mut cb, Some(uncount), sem.waiters.as_ptr().cast()) };
...
/// Takes the named-semaphore table's lock across `fork`.
pub fn fork_prepare() {
    TABLE_LOCK.acquire();
}

/// Releases the table's lock in the parent after `fork`.
pub fn fork_parent() {
    TABLE_LOCK.release();
}

/// Clears the table's lock in the child after `fork`.
pub fn fork_child() {
    TABLE_LOCK.reset();
}
...
fn open(path: *const u8, flags: c_int, mode: c_uint) -> Result<usize, c_int> {
    ...
    let ret = unsafe {
        syscall::syscall4(nr::OPENAT, AT_FDCWD, path.addr(), flags as usize, mode as usize)
    };
    errno::decode(ret)
}
...
fn map_file(fd: usize) -> Result<(u64, *mut Sem), c_int> {
    ...
    // is atomic.
    let ret = unsafe {
        syscall::syscall6(nr::MMAP, 0, size_of::<Sem>(), PROT_READ_WRITE, MAP_SHARED, fd, 0)
    };
...
/// mapping.
fn open_or_create(path: &PathBuf, flags: c_int, mode: c_uint, value: c_uint) -> Result<(u64, *mut Sem), c_int> {
    ...
    loop {
        ...
        let _ = unsafe { syscall::syscall2(nr::CLOCK_GETTIME, CLOCK_REALTIME as usize, (&raw mut now).addr()) };
        let mut tmp = [0_u8; 40];
        ...
        let wrote = unsafe { syscall::syscall3(nr::WRITE, fd, (&raw const initial).addr(), size_of::<Sem>()) };
...
pub unsafe extern "C" fn sem_open(
    ...
    let existing = TABLE_SEM.iter().enumerate().position(|(i, entry)| {
        ...
        i != slot
            && !sem.is_null()
            && sem != reserved()
            && TABLE_INO.get(i).is_some_and(|n| n.load(Ordering::SeqCst) == ino)
    });
    let (index, sem) = match existing {
        Some(i) => {
            ...
            (i, TABLE_SEM.get(i).map_or(map, |e| e.load(Ordering::SeqCst)))
        }
        None => (slot, map),
    };
    if let (Some(entry), Some(inode), Some(refs)) = (TABLE_SEM.get(index), TABLE_INO.get(index), TABLE_REFS.get(index)) {
...
pub extern "C" fn sem_close(sem: *mut Sem) -> c_int {
    ...
    let (Some(entry), Some(inode), Some(refs)) = (TABLE_SEM.get(i), TABLE_INO.get(i), TABLE_REFS.get(i)) else {
        return 0;
    };
