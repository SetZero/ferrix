...
type Hook = Option<unsafe extern "C" fn()>;
...
/// the stack. The environment must follow `argv`'s terminating null.
...
pub unsafe extern "C" fn __libc_start_main(
    ...
    // each pair of bounds, and nothing has run that could depend on them.
    unsafe {
        run(
