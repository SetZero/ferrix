...
|---|---|---|
| present | 887 | defined by `libferrousli.a`, or a macro the standard specifies as one and `include/` defines |
...
| absent | 349 | not there |

356 interfaces are missing in one of the last four ways. 48 of them are
...
| Threads and scheduling | 145 | 43 | 35 | 17 | landing `ferrousli-threads`: semaphores and C11 `threads.h` 6; cancellation, `pthread_cancel` and `pthread_testcancel` with their cancellation points 8; the `clock` variants of the condition, mutex, read-write lock and semaphore waits 2; `pthread_atfork` 1 |
