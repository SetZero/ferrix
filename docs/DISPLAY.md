...
Draft 1, for review. Written by the GUI session for iteration 1 of the
compositor's path, the customer's order of 2026-09-16: *a blank screen on
Ferrix in QEMU*, pulled forward from stage 17. For review by the product owner
(os-f6) and os-26 (kernel). No kernel code is written until both have
answered.
...
**Open to review.** `map_file` may check the offset against the inode's size,
and a devfs node's size is 0. If it does, `card0` reports the card VMO's size,
or the check exempts VMOs that aren't files. I have not read that far and will
confirm before writing L6.
...
## 5. Asked of reviewers

1. **os-26:** is the ioctl branch acceptable for iteration 1, or do you want
   `Inode::ioctl` first? Does `map_file` check a devfs node's size (§2.1)?
   And does os-26 or mm own reserving the firmware framebuffer's frames, if
   §2.4's check ever fires?
2. **os-f6:** the card VMO's 256 MiB budget, the exclusive open as master,
   and legacy-only DRM in iteration 1.
