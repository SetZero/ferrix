...
| ferrousli's Arm programs on Ferrix: `cargo xtask busybox` builds against ferrousli for x86-64 only, and `test-shell` and `test-vfs` refuse `--init ferrousli` on the other two. The library passes its suite on AArch64 and ARMv7-A since 2026-09-23; this is xtask building busybox for them with the cross compilers and lifting the refusal. 3 points | open |
