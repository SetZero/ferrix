...
| No moving wallpaper has ever been shown on this host, because the AV1 landing's conversion step never ran here. `~/.local/share/ferrix/wallpapers` holds 17 `.fxwall` and zero `.ivf`, which is what `wallpaper::file` looks for (`MOVIE_KIND`); the one `.fxvid` there is orphaned, kept under the name the landing replaced, and nothing reads it any more. Ten of the 17 `.fxwall` were cut from `.mp4` or `.webm` sources before c5806e59, so they are single still frames of videos -- the directory looks full while the moving path has no input at all, and `run-compositor` has been picking a still every time. This is the half that belongs to the AV1 landing rather than to ferrix-28's `choose` soft-fail fix, which is the other half of the same 2026-09-19 finding. What it wants: re-run `cargo xtask wallpapers --from <directory>` on nazuna so the videos are kept as `.ivf`, delete the orphaned `.fxvid`, and then actually boot one and look at it, since no gate covers the moving path and nobody has seen it run since the format changed. A gate would be better than a look, and the first question is whether one can afford a video in the initramfs. 1 point | open -- wants whoever next has nazuna's display | 19 |
