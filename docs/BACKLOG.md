...
| ferrousli's wrappers for the Linux event calls stage 17's Rust event loop (calloop over the `polling` crate) needs: `epoll_create`, `epoll_create1`, `epoll_ctl`, `epoll_wait` and `epoll_pwait`, now that the kernel has epoll (d047480d); `eventfd`, `eventfd_read` and `eventfd_write` over the kernel's `eventfd2` (9ed8808f); and `timerfd_create`, `timerfd_settime`, `timerfd_gettime` and `signalfd` once the kernel has them. musl 1.2.5's headers declare all of them, but none links: `src/generated/nr.rs` does not yet have their numbers. 3 points: epoll 1, eventfd with timerfd and signalfd 1, and the tests 1, C programs on the host and one booted on Ferrix where an eventfd wakes an `epoll_wait` | os-a8 (ferrousli), the timerfd and signalfd half after os-26's kernel rows | 17 |
...
| Iteration 2, the Wayland server's kernel calls, verified from source by the GUI session: E1 `epoll_create1`/`epoll_ctl`/`epoll_wait`/`epoll_pwait`, a set waitable inside another (8, landed 2026-09-16); E2 `eventfd2` (2, landed 2026-09-16); E3 `ioctl(FIONBIO)` on sockets and pipes (1, landed 2026-09-16); the kernel side is done. No `timerfd`, no `signalfd`. Kernel session os-26 | 17 |
| The input iteration, and iteration 2's prerequisites, from `docs/INPUT.md`, approved by os-f6 2026-09-16. Input, 23 points: L1 `libs/linux-abi::input` from a committed probe (2), L2 `libs/virtio::input` (2), L3 `libs/inputctl`, the messages and the per-open queues (3), L4 `libs/virtio-input`, the driver logic (3), L5 `user/input`, devmgr's entry and the input core's task (5), L6 `/dev/input/eventN` and its evdev subset (5), L7 `compositor/evecho` and `xtask test-input` over QMP with its negative control (3); GUI session (os-e5), kernel reader for L5 and L6 open. Iteration 2's prerequisites, 14 points: E1 `epoll` with nesting (8, landed 2026-09-16), E2 `eventfd2` (2) and E3 `ioctl(FIONBIO)` (1), os-26's row above; E4 `card0`'s primary plane and properties (3, `docs/DISPLAY.md` §2.3), GUI session (os-e5). E5 `timerfd` (3) is wanted, not required, and unassigned. Landed: L1 (2). L2's protocol half is on `main` (493fd843, 0bfb1de4); its fuzz run and the switch to L1's numbers remain | 17 |
