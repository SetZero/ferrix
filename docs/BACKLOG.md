...
| WHPX: `/sbin/blk` dies at its first common-config read on Windows (QEMU 11.1.0 under WHPX; main 2da229f9). In `ferrix_virtio::pci::negotiate` (`libs/virtio/src/pci.rs:185`, `movl 0x4(%rax),%esi` reading `device_feature` right after writing `device_feature_select`, `rax` its mapped aperture 0x7fffff6fe000) the kernel takes vector 14 with error code 12 (user, reserved bit) at address 0, the same frame every time. One blk driver of two dies per boot, so devmgr reports a failure and stage 10's driver check panics FX-1005. Reproduced by `run` and by `test-boot --init ferrousli --net --zinc --accel whpx` (3 of 3); clean under TCG on the same PC with the window, and under TCG and KVM on nazuna; the display, net and zinc are not involved. Root cause first — WHPX's MMIO emulation of a ring-3 access, or how the aperture is mapped — then a fix or a written workaround (`--accel tcg` meanwhile). 3 points to root cause (GUI session's findings of 2026-09-16) | os-26, after the eventfd fix and before the 15-site conversion; os-02 for the aperture mapping | The customer's own `run` on Windows panics at boot |
...
| Stage 10's driver check names a dead driver: `published_by_devmgr` in `kernel/src/block_ring/driver_check.rs` waits for the disk and never looks at the driver, so a blk killed by a signal reads "a disk devmgr reported started is not in the registry" (FX-1005). It should say the driver died and how, as `published` does for its own child. 1 point | os-02, after the SMMU skip | 10 |
...
| Unlanded on `ports-curl-btop`, os-12's, for after the release: **the time of day and a seeded random generator.** `BootInfo` version 5 carries firmware's `GetTime` and 32 bytes of `EFI_RNG_PROTOCOL`; `libs/crng` (ChaCha20 with fast key erasure, RFC 8439 and OpenSSL vectors) seeded from them and from `RDSEED`/`RDRAND` or `RNDR` replaces the xorshift behind `getrandom`, `/dev/urandom` and `AT_RANDOM`; `CLOCK_REALTIME` starts at firmware's time. It booted on x86_64, aarch64 and armv7a with the clock read and 256 or 512 bits credited, and its host tests pass; nothing else is gated. With it, `test-net` gains an HTTPS program against Mbed TLS's test server in the guest, which has not run yet. Then **git**, not started. 13 points for the first, git unsized | os-12 |
...
| `getrandom` seeded from virtio-rng into a real generator; a real-time clock read from the RTC and `/dev/rtc`. curl's HTTPS needs both: a guest fetch was refused because the certificate's validity "starts in the future" at 1970, and a TLS session key from today's `getrandom` would be predictable. Done includes an HTTPS program in `test-net` that is hermetic, served by `xtask` or by an Mbed TLS server inside the guest | open |
