...
| Item | Owner |
|---|---|
| `test-compositor`'s driver-restart check failed once on 2026-09-26, x86-64 under KVM, gating the cursor-shape fix (9b1339a0, which touches only `wl_cursor_shape_device_v1`): "the display driver was not restarted under the compositor: the compositor saw its card go: `the card went away` 0 of 1 times". The restart itself happened -- devmgr started the driver again twice (restart 1 at 5.50 s, restart 2 at 5.76 s) and hyprix said "the card is back; drawing on it again" at 6.02 s -- but hyprix never printed `the card went away`, so either it missed the first loss between two restarts 260 ms apart or the line is not printed on that path. The rerun of the same tree passed every boot. Kept: `~/.local/share/ferrix/logs/cursor-shape-compositor-fail1.log` on nazuna, lines 350-388. 1 point | open |
| A second way to the quiesce's FX-1004, found the day main's fix landed: `Endpoint::write` holds its reference to the driver's end until after it has woken the reader, so a driver that reads READY, closes its handle and is quiesced at once can find its end still alive for a moment (4 of 30 loaded boots on a branch without main's fix). A fix -- `Endpoint::peer_gone`, a quiesce waiting 20 ms for a dead driver's end, with a check that holds the end deliberately -- is commit 6afa79fc on branch `dk1-console/tx-fix`, not landed: measure it against main's fix first (loaded boots, `docs/BACKLOG.md`'s FX-1004 row) | unowned, found by the DK1 session |
