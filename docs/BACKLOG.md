...
* ferrousli (`ferrix-ce`): 154 symbols were undefined at busybox's first link. Area 1, the system-call wrappers, is on develop, and the 31 names areas 5-7 may leave as stubs are in `ferrousli/src/stubs.rs`. Before busybox links: termios (area 2); users and groups, `crypt`, utmp and syslog (area 3); `scanf`, `popen`, `system`, `mkstemp`, `realpath`, `execle`, `execlp` and `daemon` (area 4).
