...
| The rest of the init, which is what is left of stage 15. **A working init landed 2026-09-26 (L4):** `/sbin/init` over `libs/svc`'s manager mounts `/run` and cgroup2, runs each service in a cgroup of its own from its first instruction, reaps, restarts by `Restart=` and the start limit, gives the console a getty whose shell leads its own session with the console as its controlling terminal, and on `SIGTERM` stops everything in reverse order and powers off; `cargo xtask test-init` types at that shell on all three architectures (`docs/INIT.md` §15, stages one and two). No image boots it yet: `cargo xtask run` and every other gate still start their program as pid 1 (`kernel/src/init.rs`) until L10 moves them over, and `test-jobs` moves onto a getty with it. **Designed 2026-09-23 in `docs/INIT.md`**: 67 points to L10, and 18 more later (L11 to L13); stage 13's cgroups for it, C1 to C5, C7 and C8, are in (G1 to G5, `docs/CGROUPS.md` §7.1). 41 of the 67 points are left (`docs/INIT.md` §16): L3, L1 and L2 landed 2026-09-24, L4 on 2026-09-26; L6, `svc` and the control socket, is next, with L5, L7 and L9 beside it | open | 15; stage 13's cgroups for its first boot are in |
