...
| The rest of ferrousli's POSIX.1-2024 gap, by area in `docs/POSIX-2024.md`: 255 interfaces, 91 points, none written on a branch since the three branches of 2026-09-13 landed. The largest parts: complex arithmetic (66 interfaces, 8 points); every `long double` form of `math.h` (59, 8), the only part of the math library left; realtime (29, 11: `aio.h`, `mqueue.h`, timers, `shm_open`); spawning (25, 7); locales and messages (24, 15: `gettext`, `iconv`, `catgets`, `strfmon`, of which os-12 takes `catgets`); processes and the system (10, 8, of which os-12 takes `pathconf`); users and databases (`ndbm.h`, 9, 3); cancellation and the `clock` waits (8, 11, of which os-12 takes cancellation); terminals (7, 3); and POSIX.1-2024's declarations in musl 1.2.5's headers (3). Each landing updates the document's tables | os-a8 (ferrousli; was os-50) |
