...
| Ring-3 virtio-blk reading sectors: `devmgr` (waiting on stage 9's `process_create`/`process_start`, which follow ferrix-a5's start argument, ferrix-4b; process observers are in); the block ring's kernel side is on develop up to a published disk (the devfs node from HELLO; reset before release is `devmgr`'s, the P1 row); a sector read through the ring by a fake driver on `ferrix-rt` in the boot check, then the virtio-blk driver process on `ferrix-rt` reading sectors through a translated domain | ferrix-d9, with ferrix-61 | Stage 11's mount and exit wait on it; the customer declined a kernel-side disk path |
