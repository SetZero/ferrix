...
| open | Stage 8's list (14), `AF_UNIX` 3b the in-flight cycle pass (3) and 3c credentials (2), stage 10's BAR trust (8, first piece landed), `timerfd` (3), dynamic linking (39, kernel half, `DT_RUNPATH`, `DT_FINI_ARRAY`, `DT_FINI` and x86-64 initial-exec TLS done 2026-09-21; the exit's first half, Debian's glibc busybox on all three architectures, met 2026-09-21, and its second half, the same busybox on ferrousli's loader and libc.so.6, met on x86-64 the same day; general-dynamic TLS and dlfcn.h 2026-09-22; 6 points left, and ferrousli's AArch64 and ARMv7-A port, ≈ 34, which the customer put inside this stage), stage 12 |
...
| Shared futexes across processes. `futex` keys every wait by the address space and the address (`kernel/src/syscall/futex.rs`), so a waiter and a waker in two processes sharing a `MAP_SHARED` page never meet: a `PTHREAD_PROCESS_SHARED` mutex, condition variable or barrier after `fork`, or a robust shared mutex, sleeps until its timeout or for ever. The file's header gives the reason -- "nothing here shares memory between address spaces yet" -- which stopped being true with stage 8's shared mappings. Linux keys a futex without `FUTEX_PRIVATE_FLAG` in a shared mapping by its backing object and offset; here that object is the VMO, which stage 6 gave anonymous memory for exactly this. Wants a boot check with a negative control: a forked child waits on a word in a shared anonymous mapping and the parent wakes it. Found 2026-09-23 in the SysML sync; ferrousli's `tests/c/thread/barrier.c` does this, but only on the host | open | 7 |
...
| The DK1's LTDC display and USB HID as the hardware variant of stage 17. **The display is done, 2026-09-23** (`docs/DISPLAY.md` §6): the kernel clocks and muxes the LTDC and I2C1 and publishes a device-tree node, `user/ltdc` drives the LTDC and the SiI9022 bridge, buffers are contiguous and cache-cleaned for it, and `hyprix` ran on the board at 1280x720 over HDMI. Left: USB HID (the board has no input Ferrix drives), other modes than 720p60, hotplug | 17, P3 |
