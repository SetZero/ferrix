...
| `process_start` must not fail after moving the bootstrap: split `sched::spawn_user` into a fallible prepare and an infallible launch (with undo on drop), deleting the move-back that re-inserts the handle under a value the caller can't learn | os-b6, os-94 reviews, os-a6 for the scheduler | 9, 10 |
...
| The debt the roadmap names: fuzz targets for `cpio`, `fdt`, `acpi`, `virtio`, `linux-abi` | ferrix-e5 (`cpio`, `fdt`, `acpi`) |
