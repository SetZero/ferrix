...
| Iteration 2, the Wayland server's kernel calls, verified from source by the GUI session: E1 `epoll_create1`/`epoll_ctl`/`epoll_wait`/`epoll_pwait`, a set waitable inside another (8, landed 2026-09-16); E2 `eventfd2` (2); E3 `ioctl(FIONBIO)` on sockets and pipes, `ENOTTY` today (1). No `timerfd`, no `signalfd`. Kernel session os-26 | 17 |
