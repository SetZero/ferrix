...
| os-05 | zinc: `zinc-next`, the port of zsh's C runtime (36k lines; it compiles and passes `check --zinc`), landing in five slices that keep the oh-my-zsh gate green: S1 core state, S2 expansion, S3 signals and jobs and S4 execution are on main, S5 prompt, builtins and start-up follows (19 of the 40 points); the other 21 are the remaining builtins, the history ring, ZLE, completion and modules, and the swap to `zinc`; `cargo xtask busybox` and its staleness rule |
