...
| open | Stage 8's list (14), `AF_UNIX` 3b the in-flight cycle pass (3) and 3c credentials (2), stage 10's BAR trust (8, first piece landed), `timerfd` (3), dynamic linking (39, kernel half 3 of 5 done 2026-09-20), stage 12 |
...
| sshdt at boot: `run --forward <n>:22` could start `sshdt` itself, with the host user's public keys as `/.ssh/authorized_keys`, and keep one host key between boots so `ssh` stops warning that it changed. Today it is started by hand (README). 2 points | open |
