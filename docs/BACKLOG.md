...
| Input L4, `libs/virtio-input`, the driver logic (3), unlanded at the wind-down of 2026-09-17: nazuna side ref `os-e5/input-l4` at e42623ea, a WIP commit that was never compiled or tested. Written: `Driver` over `Transport`/`DevicePages`/`EventArea` as `libs/virtio-gpu` has it (bring-up without `DRIVER_OK` until READY, HELLO from every event type the device declares, 64 event buffers, batching at the last `SYN_REPORT` within 64 events, a report reaching 255 events closed by the driver, `Teardown::Wedged` when the reset does not finish), and a fake device copying QEMU 9.2.4's keyboard, mouse, tablet and multi-touch tables with 17 tests. Two departures from `docs/INPUT.md` wait for review: the driver drops what the core would not publish, and it splits an over-long report. The crate is deliberately not a workspace member yet. **First on the next machine:** fast-forward the branch onto main, add `libs/virtio-input` to the workspace, run `cargo test -p ferrix-virtio-input` and clippy on nazuna and fix what breaks; `src/tests.rs` names a `tests/batch.rs` still to write. Then the `virtio_input_driver` fuzz target and seeds, registration (ROADMAP crate row, sysml part, `docs/generated`, INPUT.md §5), and the gates: crate tests, ten minutes of fuzzing, `cargo xtask check`, `test-boot --arch armv7a --smp 2`. For L5: how the core learns a driver broke is undefined, and the glue must call `on_interrupt` again while `Drained.more` is set. GUI session (os-e5) | 17 |
| The compositor's tiny-skia renderer, `compositor/render` (5, stage 18's list), unlanded at the wind-down of 2026-09-17: nazuna side ref `os-e5/compositor-render` at 5e2a9d12, a WIP commit that was never compiled, tested or gated. Written: a Smithay-free `Canvas` over a tiny-skia pixmap (`tiny-skia =0.12.0`, default features off, `std` and `simd`: no C, no build script) with `clear`, `fill`, `border` and `composite` (ARGB8888 source-over, XRGB8888 copy) drawn only inside a non-overlapping `Damage` region, `present` into an XRGB8888 target of any stride, `render` of one monitor from `compositor/layout`'s output with border colours from `compositor/config`, `damage_between` two layouts, two pattern clients, a run-length expected-image format (`tests/data/*.xrle`, written only with `COMPOSITOR_RENDER_BLESS=1`, `COMPOSITOR_RENDER_PPM=<dir>` to look at frames), crate docs on how a Smithay 0.7 `Renderer`/`Frame`/`ImportMem` or a from-scratch compositor wraps it, and 21 tests including the byte-exact golden image of two tiled clients at 1024x768 and its one-pixel negative control. **First on the next machine:** fetch the branch into a fresh worktree, run `cargo test -p compositor-render` in `compositor/` with its own target dir and fix until it compiles (unverified: `compositor_layout::Dispatcher::from_bind`, the style test's config text, tiny-skia's rounding against the hand-computed blends); only then bless the golden image, look at it through the PPM dump, commit it, and run fmt, clippy and test in `compositor/`, the `x86_64-unknown-linux-musl` cross-build and `cargo xtask check`; add the `compositor/README.md` entry and stage 18's paragraph. GUI session (os-e5) | 18 |
