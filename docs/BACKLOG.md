...
| Only `init/`, or `libs/svc` | `cargo xtask check`, which runs `init/`'s formatting, clippy and tests by default and `libs/svc`'s with the host's, then `cargo xtask test-init --arch all`, which boots `/sbin/init` as pid 1 and types at the shell its getty gives |
...
| ferrix-90 | Audio (`docs/AUDIO.md`), moved forward by the customer on 2026-09-26: L1 to L7, the driver, the core and `test-audio` (24), then the sound server |
...
| `test-vfs --init ferrousli` command 21 (F-37's kernel memory: files made under a 256 KiB `memory.max`, then removed) said `memory.current 512 after` where `back to 0` is wanted: 512 bytes of kernel heap still charged to the cgroup after its files were removed and its shell had gone. Seen three times on 2026-09-26 on x86-64 under TCG: twice in a row on init-l4's gate (e59400ee on main 0d62cbc9), whose kernel change was only `/proc/<pid>/stat`'s rendering -- main plus that change passed the row, and main alone passed it 3 of 3 -- and once in the OOM kill's gate (init-oom-kill, before rebase), whose rerun passed. So it is intermittent, and not either change's. Where to start: what the shell's last lookups leave charged in the dentry cache (CGROUPS.md §7.1 says those stay charged until evicted), and whether it is always one 512-byte object. Kept: `~/ferrix-logs/f37-memory-current/2026-09-26-vfs-ferrousli-x86_64-e59400e.log` and `~/ferrix-logs/init-oom-kill/vfs-x86-1.log` on nazuna | unowned | 13 |
...
| FX-0905, seen once on 2026-09-26: `test-shell --arch x86_64 --init ferrousli` under TCG panicked in the boot's quota self-check, "tasks gone and still counted in their jobs' processor load", at 5.84 s, before any program ran. The tree was init-l4's gate (3e9503d3, on main 7c8dff4f, whose one kernel change is `/proc/<pid>/stat`'s rendering, which the check does not reach); the same row passed on its rerun, and every other boot of that gate passed. So a task's share of its job's processor load can outlive the task for a moment; `CpuQueue::follow_group_share` and the job's load as tasks come and go (`kernel/src/object/quota.rs`) are where to start. Kept: `~/ferrix-logs/fx0905/2026-09-26-shell-ferrousli-x86_64-3e9503d.log` on nazuna, the panic at its FERRIX-PANIC line | unowned | 13 |
| The rest of the init, which is what is left of stage 15. **L1 to L7 and L9 are in (2026-09-26), and L8's kernel half:** `/sbin/init` over `libs/svc` runs each service in a cgroup of its own, reaps, restarts, gives the console a getty whose shell leads its own session, and powers off; `/bin/svc` drives it over `/run/ferrix/control` (root-only changes by `SO_PEERCRED`, a per-unit log, reload, enable, set-property, top); `Type=notify` and `forking` readiness; `.socket` activation with `LISTEN_FDS` and `Accept=yes` instances; `MemoryMax=`, `TasksMax=`, `OOMPolicy=` over stage 13's scoped OOM kill, scopes and `Delegate=`. `cargo xtask test-init` types all of it at the prompt on all three architectures (`docs/INIT.md` §15, §16). Left: **L8's init side** (8 points: pid 1 takes its bootstrap channel, `Type=native` services and the directory's OPEN, CONNECT and REFUSED, `port_fd` in the loop), and **L10** (6: `cargo xtask run` and the desktop boot init; hyprix stops being pid 1 and makes a scope per client; `test-jobs` moves onto a getty). L9's gate as designed names sshd, which needs a `LISTEN_FDS` patch to the sshdt port (x86-64 only); activation is shown with `nc` instead. 14 of the 67 points are left | open | 15; stage 13's cgroups for its first boot are in |
...
| `cargo xtask check`'s compositor tests flaked once on 2026-09-26: `two_clients`' `the_same_frame_is_drawn_on_a_gpu` found "2087572 channels of the GPU's frame are more than 3 from the expected image" -- nearly the whole 1080p frame wrong, not a few edge pixels -- on branch `audio-l1-abi` (main 7c8dff4f plus `libs/linux-abi::sound`, which hyprix links and does not call), with the host at a load of about 15 from other sessions. The test alone on the same tree passed in 8 s. A frame that wrong looks like a GPU shared with another session's work, or a read of the frame before the GPU finished, rather than a rendering difference; start at how the test waits for its frame. Kept: `~/ferrix-logs/audio-l1-check-gpu-frame-1.log`. | unowned, found by ferrix-90 |
