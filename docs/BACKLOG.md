...
| LEDs: `write` to `/dev/input/eventN` of `EV_LED` events, carried to the driver by an additive `inputctl` message (`STATUS`, core → driver). **Done for USB keyboards, 2026-09-23** (`docs/INPUT.md` §3.3, §7.4): the core keeps the state and a USB keyboard's output report lights it. Left: virtio-input's status queue (it takes STATUS and drops it, so QEMU's keyboard stays dark); `write` of types other than `EV_LED` and `EV_SYN`, which Linux injects and Ferrix answers `EINVAL`; the LED events passed to the node's readers; a compositor writing the LEDs as its lock state changes (hyprix) | 17 |
