...
| ferrousli's link-breakers, from `docs/POSIX-2024.md`: `assert` fails to link (`__assert_fail` is missing), as do `fpclassify` for every type and `isinf`, `isnan`, `isnormal`, `isfinite`, `signbit` and the six comparison macros (`isgreater` to `isunordered`) for `long double` (`__fpclassify*`, `__signbit*`); and `crypt` returns `"*"` for a two-character salt, the kind POSIX requires it to take, where every other C library computes the traditional DES hash. One landing, 5 points | os-50 | ferrousli |
